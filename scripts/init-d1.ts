// 独立脚本：初始化 D1 数据库（建表 + 导入 mock 数据）
// 直接用 fetch 调 Cloudflare D1 HTTP API，无需 wrangler
...
const merchants = [
  ...
  { id: "m03", name: "肯德基", floor: "1F", location: "1F-105", category: "快餐", emoji: "🍗", manager: "王五", contact: "138-0000-0003", area: 200, openHours: "07:00-23:00", verified: true, signedIn: false, absent: false },
  ...
  { id: "m12", name: "外婆家", floor: "4F", location: "4F-428", category: "中餐", emoji: "🥟", manager: "褚四", contact: "138-0000-0012", area: 220, openHours: "11:00-21:30", verified: true, signedIn: false, absent: true },
  ...
  { id: "m17", name: "海澜之家", floor: "2F", location: "2F-222", category: "服装", emoji: "👔", manager: "杨九", contact: "138-0000-0017", area: 160, openHours: "10:00-22:00", verified: false, signedIn: false, absent: true },
  ...
  { id: "m29", name: "GAP", floor: "2F", location: "2F-220", category: "服装", emoji: "👕", manager: "魏一", contact: "138-0000-0029", area: 200, openHours: "10:00-22:00", verified: true, signedIn: false, absent: false },
  ...
  { id: "m34", name: "绿茶餐厅", floor: "3F", location: "3F-320", category: "中餐", emoji: "🥬", manager: "邹六", contact: "138-0000-0034", area: 180, openHours: "11:00-21:30", verified: true, signedIn: false, absent: false },
...
async function main() {
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS roll_call_rules (
      ...
      daily_end_time TEXT DEFAULT '11:00',
      absent_threshold TEXT DEFAULT '11:00',
      remind_before INTEGER DEFAULT 10,
      holidays TEXT DEFAULT '[]',
      weekly_off TEXT DEFAULT '[0]',
      enable_auto_absent INTEGER DEFAULT 1
    )`,
  ];

  for (const sql of tables) {
    const r = await query(sql);
    console.log(r.success ? "  OK" : `  FAIL: ${JSON.stringify(r.errors)}`);
  }

  const indexes = [
    "CREATE INDEX IF NOT EXISTS idx_attendance_merchant ON attendance_records(merchant_id)",
  ...
  for (let i = 0; i < merchants.length; i += 10) {
    ...
    const sql = batch.map((m) =>
      `INSERT OR REPLACE INTO merchants (id, name, floor, location, category, emoji, manager, contact, area, open_hours, verified, avatar, signed_in, absent, signed_at) VALUES ('${m.id}', '${esc(m.name)}', '${m.floor}', '${m.location}', '${m.category}', '${m.emoji}', '${esc(m.manager)}', '${esc(m.contact)}', ${m.area}, '${esc(m.openHours)}', ${m.verified ? 1 : 0}, '', ${m.signedIn ? 1 : 0}, ${m.absent ? 1 : 0}, '${m.signedAt || ""}')`
    ).join("; ");
    const r = await query(sql);
    console.log(`  merchants ${i}-${i + batch.length - 1}: ${r.success ? "OK" : "FAIL"}`);
  }

  console.log("\n=== 3. 插入默认规则 ===");
  const r3 = await query(
    `INSERT OR REPLACE INTO roll_call_rules (id, daily_start_time, daily_end_time, absent_threshold, remind_before, holidays, weekly_off, enable_auto_absent) VALUES (1, '10:00', '11:00', '11:00', 10, '["2026-01-01","2026-02-10","2026-05-01","2026-06-19","2026-10-01"]', '[0]', 1)`
  );
  console.log(r3.success ? "  OK" : `  FAIL: ${JSON.stringify(r3.errors)}`);
